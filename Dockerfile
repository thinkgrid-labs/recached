...
# Install CA certificates in case Liyab ever needs to make outbound HTTPS requests
RUN apt-get update && apt-get install -y ca-certificates && rm -rf /var/lib/apt/lists/*

# Copy the compiled binary from the builder stage
COPY --from=builder /app/target/release/server-native /usr/local/bin/liyab-server
